...
- Operate on a **sprint branch** (e.g., `sprint/{sprint_id}`) checked out from the default branch at pipeline start
...
- Commit changes to the sprint branch with a structured commit message including `pipeline_id`, `task_id`, and execution traceability metadata
- Not push the branch — the Sprint Controller (close-out) owns the push and PR creation
...
| Verifier (fail) → Fixer | Auto-advance (no gate) |
| Fixer → Verifier | Auto-advance (no gate) |
...
1. Create a sprint branch: `sprint/{sprint_id}` from the current default branch HEAD
...
2. Push the sprint branch to the remote
3. Open a GitHub Pull Request from `sprint/{sprint_id}` → default branch
...
```
Title: [sprint/{sprint_id}] {sprint_goal}
...
Started: {created_at} | Branch: sprint/{sprint_id}
```
...
The existing maximum of 3 Fixer attempts (MAX_FIXER_ATTEMPTS) is retained. After 3 consecutive Fixer cycles without a Verifier pass, the pipeline transitions to `failed` and a human escalation notification is sent. The sprint branch is not deleted; the human can inspect the state, intervene, and trigger a manual handoff.
...
Planner — produces phase plan
        │ (auto-advance)
        ▼
Sprint Controller (setup) — produces sprint plan + brief + creates sprint branch
        │ (auto-advance)
        ▼
Implementer — writes code on sprint branch, commits
        │ (auto-advance)
        ▼
...
PASS          FAIL
  │             │ (auto-advance)
  │             ▼
  │           Fixer — produces corrected code, commits
  │             │ (auto-advance, loops back to Verifier)
  │             │ (max 3 cycles, then escalate)
  ▼             │
Sprint Controller (close-out) ◄──────────────────────┘
  — pushes sprint branch
  — opens GitHub PR
...
- Implementer as coding agent requires LLM tool-calling support (ADR-029) — more complex than current completion-based approach
